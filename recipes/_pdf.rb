...
template "#{app_root}/web.config" do
  source "#{node[:egistics][:fdc_pdf_web][:web_config]}.erb"
	...
	variables({
		...
		:storage_proxy => node[:ash_db_role] ? 'https://ap-esl-spx-01.tisa.io/PRD-ESL-WSSPX-E1/synapticWebService.asmx' : 'https://dp-esl-spx-01.tisa.io/PRD-ESL-WSSPX-E1/synapticWebService.asmx'
		})
	notifies :restart, "iis_pool[#{config[:pool][:name]}]"
end
