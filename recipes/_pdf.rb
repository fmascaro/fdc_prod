...
zip_path = win_friendly_path(
             ::File.join(
               node[:egistics][:fdc_pdf_web][:artifact_path],
               node[:egistics][:fdc_pdf_web][:artifact_filename]
              )
            )

web_root = win_friendly_path(
             ::File.join(
               node[:egistics][:fdc_pdf_web][:docroot],
               node[:egistics][:fdc_pdf_web][:web_directory]
              )
            )
...
app_root = win_friendly_path(
            ::File.join(
               node[:egistics][:fdc_pdf_web][:docroot],
               node[:egistics][:fdc_pdf_web][:app_directory]
              )
            )
...
site_name = node[:egistics][:fdc_pdf_web][:site_name]
app_name = node[:egistics][:fdc_pdf_web][:app_name]
...
#Config variables
config = {
	:baseapp => (node[:egistics][:fdc_pdf_web][:baseapp] ? true : false),
	:zipfile => {
	...
	:pool => {
	:name => node[:egistics][:fdc_pdf_web][:app_pool],
	:private_mem => 600000,
	:recycle_at_time => node[:egistics][:fdc_pdf_web][:recycle_pool] ? ("01:" + minutes + ":00") : nil,
	:identity => node[:egistics][:fdc_pdf_web][:pool_identity]
	},
	:app => {
	:physical_path => app_root,
	:app_pool => node[:egistics][:fdc_pdf_web][:app_pool],
	:web_config_source => node[:egistics][:fdc_pdf_web][:config_filepath]
	},
	:log => {
	:path => node[:egistics][:fdc_pdf_web][:log_path],
	:max_size => "3MB"
	}
}
...
iis_pool node[:egistics][:fdc_pdf_web][:site_pool] do
	runtime_version '4.0'
	pipeline_mode :Integrated
	idle_timeout '00:00:00'
	recycle_after_time '00:00:00'
	pool_username node[:egistics][:fdc_pdf_web][:pool_identity]
	action [:add, :config]
	notifies :run, "execute[config_#{node[:egistics][:fdc_pdf_web][:site_pool]}]", :immediately
end
...
iis_site site_name do
	site_id 7
	application_pool node[:egistics][:fdc_pdf_web][:site_pool]
	path web_root
	bindings node[:egistics][:fdc_pdf_web][:binding]
	action [:add, :start]
...
iis_pool config[:pool][:name] do
	runtime_version node[:egistics][:fdc_pdf_web][:runtime_v4] ? '4.0' : '2.0'
	pipeline_mode node[:egistics][:fdc_pdf_web][:pipline_mode]
	idle_timeout '00:00:00'
	recycle_after_time '00:00:00'
...
template redirect_file do
	...
	variables({
			:url => node[:egistics][:fdc_pdf_web][:redirect_url]
			})
end
...
windows_zipfile app_root do
	...
	not_if { ::File.exists?("#{app_root}/web.config") || node[:egistics][:fdc_pdf_web][:baseapp] }
end

template "#{app_root}/web.config" do
  source "#{node[:egistics][:fdc_pdf_web][:web_config]}.erb"
	action :create
	variables({
		...
		:storage_proxy => node[:ash_db_role] ? 'https://ap-esl-spx-01.egistics.local/PRD-ESL-WSSPX-01/synapticWebService.asmx' : 'https://dp-esl-spx-01.egistics.local/PRD-ESL-WSSPX-01/synapticWebService.asmx'
		})
	notifies :restart, "iis_pool[#{config[:pool][:name]}]"
end
...
execute "config_#{node[:egistics][:fdc_pdf_web][:site_pool]}" do
	command "#{node[:iis][:home]}\\appcmd.exe set apppool \"#{node[:egistics][:fdc_pdf_web][:site_pool]}\" /autoStart:true /startMode:AlwaysRunning"
	action :nothing
end
