...
template "#{app_root}/web.config" do
  source "#{node[:egistics][:fdc_web_service][:web_config]}.erb"
	...
	variables({
		...
		:storage_proxy => node[:tags].include?("ashburn") ? 'https://ap-esl-spx-01.egistics.local/PRD-ESL-WSSPX-E1/synapticWebService.asmx' : 'https://dp-esl-spx-01.egistics.local/PRD-ESL-WSSPX-E1/synapticWebService.asmx'
		})
	notifies :restart, "iis_pool[#{config[:pool][:name]}]"
end
