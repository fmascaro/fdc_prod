...
template "#{app_root}/web.config" do
  source "#{node[:egistics][:fdc_web_service][:web_config]}.erb"
	...
	variables({
		...
		:test_ADV3499WACHLBX_archive_db => node[:ash_db_role] ? 'SERVER=AP-FDC-SQL-02.egistics.local;DATABASE=TEST_EmdeonDemo;Trusted_Connection=True' : 'SERVER=DP-FDC-SQL-02.egistics.local;DATABASE=TEST_EmdeonDemo;Trusted_Connection=True',
		:test_UBS0001LBX_archive_db => node[:ash_db_role] ? 'SERVER=AP-FDC-SQL-01.egistics.local;DATABASE=TEST_EmdeonDemo;Trusted_Connection=True' : 'SERVER=DP-FDC-SQL-01.egistics.local;DATABASE=TEST_EmdeonDemo;Trusted_Connection=True',
		:test_HONDAADC04285673_archive_db => node[:ash_db_role] ? 'SERVER=AP-FDC-SQL-02.egistics.local;DATABASE=TEST_EmdeonDemo;Trusted_Connection=True' : 'SERVER=DP-FDC-SQL-02.egistics.local;DATABASE=TEST_EmdeonDemo;Trusted_Connection=True',
		:test_InternalHelper_admin_db => node[:ash_db_role] ? 'SERVER=AP-FDC-SQL-01.egistics.local;DATABASE=TEST_RTC_Admin3G;Trusted_Connection=True' : 'SERVER=DP-FDC-SQL-01.egistics.local;DATABASE=TEST_RTC_Admin3G;Trusted_Connection=True',
		:test_InternalHelper_archive_db => node[:ash_db_role] ? 'SERVER=AP-FDC-SQL-01.egistics.local;DATABASE=TEST_RTC_Admin3G;Trusted_Connection=True' : 'SERVER=DP-FDC-SQL-01.egistics.local;DATABASE=TEST_EmdeonDemo;Trusted_Connection=True',
		:test_DEMOLBX_archive_db => node[:ash_db_role] ? 'SERVER=AP-FDC-SQL-01.egistics.local;DATABASE=TEST_EmdeonDemo;Trusted_Connection=True' : 'SERVER=DP-FDC-SQL-01.egistics.local;DATABASE=TEST_EmdeonDemo;Trusted_Connection=True',
		:storage_proxy => node[:tags].include?("ashburn") ? 'https://ap-esl-spx-01.egistics.local/PRD-ESL-WSSPX-01/synapticWebService.asmx' : 'https://dp-esl-spx-01.egistics.local/PRD-ESL-WSSPX-01/synapticWebService.asmx'
		})
	notifies :restart, "iis_pool[#{config[:pool][:name]}]"
end
